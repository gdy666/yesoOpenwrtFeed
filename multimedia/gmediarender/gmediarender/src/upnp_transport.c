...
static void *thread_update_track_time(void *userdata) {
	...
	for (;;) {
		usleep(200000);  // 200ms
		service_lock();
		gint64 duration, position;		//分别为音乐的总长度和音乐的当前位置
		const int pos_result = output_get_position(&duration, &position);
		if (pos_result == 0) {
			if (duration != last_duration) {		//记录媒体的长度
				last_duration = duration;
				print_upnp_time(tbuf1, sizeof(tbuf1), last_duration);
				replace_var(TRANSPORT_VAR_CUR_TRACK_DUR, tbuf1);
			}
			if (position  != last_position) {
				...
				if(last_duration - last_position <= one_sec_unit){
					last_position = last_duration;
				}
				
				print_upnp_time(tbuf2, sizeof(tbuf2), last_position);
				replace_var(TRANSPORT_VAR_REL_TIME_POS, tbuf2);
			}
		}
